const fs = require('fs');

function parseCSV(content) {
  const rows = [];
  let currentRow = [];
  let currentField = '';
  let inQuotes = false;
  
  for (let i = 0; i < content.length; i++) {
    const char = content[i];
    const nextChar = content[i + 1];
    
    if (inQuotes) {
      if (char === '"') {
        if (nextChar === '"') { currentField += '"'; i++; }
        else { inQuotes = false; }
      } else { currentField += char; }
    } else {
      if (char === '"') { inQuotes = true; }
      else if (char === ',') { currentRow.push(currentField); currentField = ''; }
      else if (char === '\n' || (char === '\r' && nextChar === '\n')) {
        currentRow.push(currentField);
        if (currentRow.length > 1 || currentRow[0] !== '') { rows.push(currentRow); }
        currentRow = []; currentField = '';
        if (char === '\r') i++;
      } else if (char !== '\r') { currentField += char; }
    }
  }
  if (currentField || currentRow.length > 0) { currentRow.push(currentField); rows.push(currentRow); }
  return rows;
}

const c = fs.readFileSync('outputs/shopify_properly_grouped.csv', 'utf-8');
const rows = parseCSV(c);
const h = rows[0];
const titleIdx = h.indexOf('Title');
const handleIdx = h.indexOf('Handle');
const opt1ValIdx = h.indexOf('Option1 Value');

console.log('\n=== BIG BUD BLOOM BOOSTER ===\n');

for (let i = 1; i < rows.length; i++) {
  const title = rows[i][titleIdx] || '';
  const handle = rows[i][handleIdx] || '';
  
  if (title === 'Big Bud Bloom Booster') {
    console.log('✓ Found parent product at row', i);
    console.log('  Handle:', handle);
    console.log('  Title:', title);
    console.log('  First variant:', rows[i][opt1ValIdx]);
    console.log('\n  Variants:');
    
    // Collect all variants
    const variants = [rows[i][opt1ValIdx]];
    for (let j = i + 1; j < rows.length && j < i + 20; j++) {
      if (!rows[j][handleIdx] && rows[j][opt1ValIdx]) {
        variants.push(rows[j][opt1ValIdx]);
      } else {
        break;
      }
    }
    
    variants.forEach((v, idx) => console.log('   ', (idx + 1) + '.', v));
    console.log('\n  Total variants:', variants.length);
    break;
  }
}
